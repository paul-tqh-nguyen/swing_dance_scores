...
class testCompetitionEditViaLocalFirestoreEmulator(unittest.TestCase):
    def testCompetitionEditViaLocalFirestoreEmulator(self):
        try:
            ...
            old_competition_privacy = "private"
            ...
            edit_competition_headers = {
                "Content-Type": "application/json",
                "Authorization": "Bearer {token}".format(token=access_token)
            }
            edit_competition_body = {
                "privacy": new_competition_privacy,
            }
            edit_competition_response = requests.put(url=edit_competition_uri, data=json.dumps(edit_competition_body), headers=edit_competition_headers) 
            edit_competition_response_status_code = edit_competition_response.status_code
            self.assertEqual(200, edit_competition_response_status_code, msg="Failed to hit the endpoint at {uri} as we got the status code of {status_code}".format(uri=edit_competition_uri, status_code=edit_competition_response_status_code))
            
            # Verify that we can find the competition by ID (without authentication)
            find_competition_uri = urllib.parse.urljoin(api_base_uri_string, "competition/{competition_id}".format(competition_id=competition_id))
            find_competition_response = requests.get(find_competition_uri)
            find_competition_response_status_code = find_competition_response.status_code
            find_competition_response_json_string = find_competition_response.content
            competition = json.loads(find_competition_response_json_string)
            self.assertTrue("competitionName" in competition, msg="Competition malformed as no competition name was specified. The competition info is {result}.".format(result=competition))
            self.assertEqual(competition["competitionName"], competition_name,
                             msg="Unexpected competitionName value for the first competition created in {result}.".format(result=competition))
            self.assertTrue("creatorHandle" in competition, msg="Competition malformed as no creator handle was specified. The competition info is {result}.".format(result=competition))
            self.assertEqual(competition["creatorHandle"], test_handle, msg="Unexpected creatorHandle value for the first competition created in {result}.".format(result=competition))
            self.assertTrue("category" in competition and competition["category"]==competition_category, msg="We could not gather the competition category set (i.e. {category}) when we created the competition. The competition data is {competition_data} ".format(category=competition_category, competition_data=json.dumps(competition)))
            self.assertTrue("judges" in competition and set(competition["judges"])==set(competition_judges),
                            msg="We could not gather the judges set (i.e. {judges}) when we created the competition. The competition data is {competition_data} ".format(judges=", ".join(competition_judges), competition_data=json.dumps(competition)))
            self.assertTrue("usersWithModificationPrivileges" in competition and competition["usersWithModificationPrivileges"]==[test_handle],
                            msg="We could not gather the users with modification privileges set (i.e. {user}) when we created the competition. The competition data is {competition_data} ".format(user=test_handle, competition_data=json.dumps(competition)))
            self.assertTrue("privacy" in competition and competition["privacy"]==new_competition_privacy, msg="We could not gather the privacy set (i.e. {privacy}) when we created the competition. The competition data is {competition_data} ".format(privacy=new_competition_privacy, competition_data=json.dumps(competition)))
            self.assertTrue("competitorInfo" in competition and competition["competitorInfo"]==competition_competitor_info, msg="We could not gather the competitor information set (i.e. {competitorInfo}) when we created the competition. The competition data is {competition_data} ".format(competitorInfo=competition_competitor_info, competition_data=json.dumps(competition)))
